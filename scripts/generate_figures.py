...
def generate_homeostasis() -> None:
    ...
    if sample_vals and all(v[0] == 0.0 for v in sample_vals[0]):
    ...
    for cond, (label, color, ls) in conditions.items():
        ...
        sems = []
        for step in steps:
            vals = [v[0] for v in cond_data[cond][step]]
            arr = np.array(vals)
            means.append(arr.mean())
            sems.append(arr.std(ddof=1) / np.sqrt(len(arr)) if len(arr) >= 2 else 0.0)
        ...
        ax.fill_between(steps, means_arr - sems_arr, means_arr + sems_arr,
                        color=color, alpha=0.15)
    ax.set_xlabel("Simulation Step")
    ax.set_ylabel("Internal State Mean [0]")
    ax.set_title("(A) Homeostatic Regulation", fontsize=9)
    ax.set_xlim(0, 2000)
    ax.set_ylim(0, 1)
    ...
    for cond, (label, color, ls) in conditions.items():
        ...
        sems = []
        for step in steps:
            vals = [v[1] for v in cond_data[cond][step]]
            arr = np.array(vals)
            means.append(arr.mean())
            sems.append(arr.std(ddof=1) / np.sqrt(len(arr)) if len(arr) >= 2 else 0.0)
        means_arr = np.array(means)
        ...
        ax.fill_between(steps, means_arr - sems_arr, means_arr + sems_arr,
                        color=color, alpha=0.15)
    ax.set_xlabel("Simulation Step")
    ax.set_ylabel("Internal State Std [0]")
    ax.set_title("(B) Population Variance", fontsize=9)
    ax.set_xlim(0, 2000)
    ax.set_ylim(bottom=0)
