...
fn run_niche_experiment_json_impl(
    ...
    if snapshot_steps_json.bytes().filter(|&b| b == b',').count() >= World::MAX_EXPERIMENT_SAMPLES {
        return Err(format!(
    ...
    if snapshot_steps.len() > World::MAX_EXPERIMENT_SAMPLES {
        return Err(format!(
...
mod tests {
    ...
    fn run_niche_experiment_json_impl_rejects_excessive_snapshots() {
        let config_json =
            serde_json::to_string(&SimConfig::default()).expect("config should serialize");
        // Create a JSON array with MAX_EXPERIMENT_SAMPLES + 1 elements
        let count = World::MAX_EXPERIMENT_SAMPLES + 1;
