...
Figure~\ref{fig:phenotype}). To test whether these clusters represent
persistent ecological strategies rather than transient snapshots, we
perform temporal persistence analysis: independent $k$-means clustering
at early (step~750) and late (step~1,750) windows within 2,000-step
simulations yields an adjusted Rand index of $-$0.04, indicating that
cluster assignments are not stable across time windows at this
timescale. This suggests that while phenotypic differentiation emerges
within any given snapshot, stable ecological niches may require longer
evolutionary timescales---consistent with the modest 2,000-step
evolution effect ($\delta$=0.34) observed in single-ablation
experiments.
