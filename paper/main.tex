...
\label{tab:criteria}
\small
\begin{tabular}{@{}p{1.3cm}p{2.2cm}p{2.6cm}p{3.8cm}p{2.4cm}@{}}
\toprule
Criterion & State Variables & Ablation Operator & Coupling ($\to$ mediator $\to$ target) & Failure Mode \\
\midrule
Cell.\ Org. & $b \in [0,1]$ & Skip repair; decay only & $e \to$ repair rate $\to b$ & $b<0.1 \to$ death \\
Metabolism & $e,r,w$; graph $\mathbf{p}$ & Freeze $(e,r,w)$ & throughput $\to e \to$ boundary repair & Energy depletion \\
Homeostasis & $\mathbf{s} \in \mathbb{R}^3$ & Skip NN delta; decay only & $s_0 \to$ repair efficacy & State drift \\
Growth & $m \in [0,1]$ & Freeze $m=0$ & maturity $\to$ reproduction gate & Cannot reproduce \\
Reproduction & Population event & Skip division check & energy cost $\to$ parent $e$ & No replacement \\
Response & $\mathbf{v}$ & Skip velocity delta & movement $\to$ resource access $\to r$ & Starvation \\
Evolution & genome $\mathbf{g}$ & Copy without mutation & $\mathbf{g}$ variation $\to$ all criteria & No adaptation \\
\bottomrule
\end{tabular}
\end{table*}
...
Ablation toggles do not alter timestep duration or random event ordering;
disabled processes are simply skipped, ensuring identical RNG sequences
across conditions.
...
with bootstrap 95\% confidence intervals (2000 resamples),
...
\begin{table}[t]
\centering
\caption{Criterion-ablation results ($n$=30 independent seeds per condition;
each seed maps to one $N_T$ scalar). Normal baseline
mean: 293.1 (median: 294, IQR: 282--310). Seven one-sided tests
(each ablation vs.\ normal baseline), Holm-Bonferroni corrected at
$\alpha=0.05$. $^{***}p<0.001$, $^{*}p<0.05$.}
\label{tab:ablation}
\small
\begin{tabular}{@{}lrrrrr@{}}
\toprule
Condition & Mean & $\Delta$\% & Cliff's $\delta$ [95\% CI] & $p_{\text{corr}}$ & Sig. \\
\midrule
No Reproduction & 31.3 & $-$89.3 & 1.00 [1.00, 1.00] & $<$0.001 & $^{***}$ \\
No Response     & 34.1 & $-$88.4 & 1.00 [1.00, 1.00] & $<$0.001 & $^{***}$ \\
No Metabolism   & 46.0 & $-$84.3 & 1.00 [1.00, 1.00] & $<$0.001 & $^{***}$ \\
No Homeostasis  & 68.3 & $-$76.7 & 1.00 [1.00, 1.00] & $<$0.001 & $^{***}$ \\
No Boundary     & 121.8 & $-$58.5 & 1.00 [1.00, 1.00] & $<$0.001 & $^{***}$ \\
No Growth       & 185.6 & $-$36.7 & 1.00 [1.00, 1.00] & $<$0.001 & $^{***}$ \\
No Evolution    & 278.3 & $-$5.1  & 0.32 [0.02, 0.59] & 0.016 & $^{*}$ \\
\bottomrule
\end{tabular}
\end{table}
...
($\bar{x}$=47.7 vs.\ 31.3 at $T=2000$), with 69\% decline relative to
normal versus 89\% at $T=2000$.
...
($r=-0.85$, $p<0.001$, lag~0), consistent with the
energy$\to$boundary repair conversion pathway.
...
\paragraph{Are criteria merely engineered subsystems?}
...
First, the proxy control shows that alternative metabolism implementations
satisfying the same functional-analogy definition produce qualitatively
distinct dynamics (Figure~\ref{fig:proxy}), ruling out tautological
definitions.
...
Third, the no-evolution ablation---which runs identical reproduction
machinery but copies genomes without mutation---serves as a near-sham
control: its modest effect ($\Delta$=$-$5.1\%) demonstrates that
non-functional modifications have proportionally small effects.
