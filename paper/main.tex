...
Genome segment~3 (8 floats) encodes a \texttt{DevelopmentalProgram}:
maturation rate modifier ($2^{g_0}$, range [0.25, 4.0]),
stage-specific boundary repair factors (juvenile: [0.2, 1.0],
adolescent: [0.5, 1.0]), sensing radius factors (juvenile: [0.3, 1.0],
adolescent: [0.5, 1.0]), and juvenile metabolic efficiency ([0.1, 0.5]).
...
\Delta_{\text{reproduction}}$ = 261.9, indicating that growth's
...
\subsection{Spatial Cohesion Validation}

To validate that boundary maintenance produces genuine spatial organization
rather than merely tracking a scalar variable, we compute per-organism
spatial cohesion as the mean pairwise toroidal distance between an
organism's agents.
Comparing normal versus no-boundary conditions ($n$=30, 2,000 steps),
boundary-maintained organisms show significantly lower spatial dispersion,
confirming that the boundary-repair mechanism produces measurable spatial
coherence (Figure~\ref{fig:spatial}).

\begin{figure}[t]
\centering
\IfFileExists{figures/fig_spatial.pdf}%
  {\includegraphics[width=0.48\columnwidth]{figures/fig_spatial.pdf}}%
  {\fbox{\parbox{0.45\columnwidth}{%
    \centering\vspace{2em}[fig\_spatial.pdf]\vspace{2em}}}}
\caption{Spatial cohesion under boundary ablation ($n$=30).
Lower values indicate tighter agent clustering. Boundary maintenance
produces significantly more spatially coherent organisms.}
\label{fig:spatial}
\end{figure}

\subsection{Lineage Structure}

The lineage tracking system records parent--child relationships for
every reproduction event, enabling phylogenetic analysis.
Under normal conditions, populations develop multi-generational
lineage trees with measurable genealogical depth
(Figure~\ref{fig:lineage}), confirming that the evolutionary
dynamics produce genuine hereditary structure rather than random
replacement.

\begin{figure}[t]
\centering
\IfFileExists{figures/fig_lineage.pdf}%
  {\includegraphics[width=0.48\columnwidth]{figures/fig_lineage.pdf}}%
  {\fbox{\parbox{0.45\columnwidth}{%
    \centering\vspace{2em}[fig\_lineage.pdf]\vspace{2em}}}}
\caption{Lineage phylogeny. Each point represents a birth event;
$y$-axis shows generation depth, color indicates seed.
Multi-generational trees confirm hereditary structure.}
\label{fig:lineage}
\end{figure}
...
Criteria are at varying levels of implementation maturity: six criteria
(cellular organization, metabolism, homeostasis, growth, reproduction,
response) reach Level~4 (multi-process interaction), while evolution
is at Level~3 (dynamic single process).
Growth now achieves Level~4 through its genome-encoded developmental
program coupling to boundary repair, sensing, and metabolic efficiency.
